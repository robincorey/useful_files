...
trajfile_list = ["md_1.pbc.xtc", "md_2.pbc.xtc", "md_3.pbc.xtc"] # each of these should be a file that exists in your current dir
topfile_list = ["md_1.tpr.pdb", "md_1.tpr.pdb", "md_1.tpr.pdb" ] # a pdb or gro file. You can re-use the same one, providing the atoms are identical 
dt_traj = None  # Not necessary for trajectory formats of e.g. xtc, trr.
stride = 1   # tell pylipid to analyze every stride-th frame. Can be used to save computation memory.

lipid = "CHOL"        # residue name in the topology. Can be lipid or drug. One per script.
lipid_atoms = None    # all lipid atoms will be considered for interaction calculation. Specific atoms can be defined manually as a list if desried.
cutoffs = [0.5, 0.8]  # dual-cutoff scheme for coarse-grained simulations. Single-cutoff scheme can be
                      # achieved by using the same value for two cutoffs. Note that for Atomistic sims, [0.35, 0.6] probably works better.
